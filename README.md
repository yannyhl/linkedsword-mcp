...
npx linkedsword-mcp-server install

# Build and install the Studio plugin
npx linkedsword-mcp-server install --plugin
```
...
npx linkedsword-mcp-server auth set --api-key=<key> --user-id=<creator-id>

# Or ROBLOSECURITY cookie (Decal uploads only)
npx linkedsword-mcp-server auth set --cookie=<cookie>
```
